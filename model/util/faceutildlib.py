...
class FaceUtil:
    ...
    def get_main_face_location(self, image):
        face_location_list = self.get_face_location(image)
        if (len(face_location_list) == 0):
            return None
        elif (len(face_location_list) == 1):
            return face_location_list[0]
        face_area = [(det[0] - det[2]) * (det[1] - det[3]) for det in face_location_list]
        max = face_area[0]
        max_index = 0
        for i in range(1, len(face_area)):
            if (face_area[i] > max):
                max = face_area[i]
                max_index = i
        return face_location_list[max_index]
